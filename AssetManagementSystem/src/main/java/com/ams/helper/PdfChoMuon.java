...
@Override
 protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer, HttpServletRequest request,
  ...
  String filename = Common.getDateCurrent(ParamsConstants.CD_FULL_FORMAT) + "_LenhDieuDong.pdf";
  String atribute = "attachment; filename="+filename;
  response.setHeader("Content-Disposition", atribute);
  ...
  if(Common.isNotEmpty(exObject.getBorrowCoupon().getMovement_sticker()))
  {
	  phrase.add(new Chunk("Số:"+exObject.getBorrowCoupon().getNumber_no().trim()+"/"+exObject.getBorrowCoupon().getMovement_sticker()+"/CĐ",  ItextVietnameseFont.VietNameseFont(ItextVietnameseFont.TIMESNEWROMAN_NORMAL, 10)));
  } else {
  ...
  String code_print = exObject.getBorrowCoupon().getMovement_sticker();
  if(exObject.getBorrowCoupon().getCmpn_client().getCompany_cd().trim().equals(exObject.getBorrowCoupon().getCmpn_master().getCompany_cd().trim())) {
	  // first row
	  phrase = new Phrase();
	  ...
	  if(code_print.equals("M") || code_print.equals("")  || code_print.equals("CM"))
	  {
		  // first row
		  phrase = new Phrase();
		  phrase.add(new Chunk("BÀN GIAO",  ItextVietnameseFont.VietNameseFont(ItextVietnameseFont.TIMESNEWROMAN_BOLD, 10)));
  ...
  if(Common.isNotEmpty(exObject.getBorrowCoupon().getDept_master().getDept_name()))
  {
	  phrase.add(new Chunk(" - " + exObject.getBorrowCoupon().getDept_master().getDept_name(),  ItextVietnameseFont.VietNameseFont(ItextVietnameseFont.TIMESNEWROMAN_NORMAL, 10)));
  ...
  if(Common.isNotEmpty(exObject.getBorrowCoupon().getDept_client().getDept_name()))
  {
	  phrase.add(new Chunk(" - " + exObject.getBorrowCoupon().getDept_client().getDept_name(),  ItextVietnameseFont.VietNameseFont(ItextVietnameseFont.TIMESNEWROMAN_NORMAL, 10)));
  ...
  {
	  MovementAssetModel ex = exObject.getLstBorrow().get(i);
	  phrase = new Phrase();
