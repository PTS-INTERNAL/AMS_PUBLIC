...
import com.ams.dao.AssetGeneralMovementDao;
...
import com.ams.dao.borrow.MovementCouponUpdateDisApproveDeptDao;
...
@Controller
@RequestMapping("/MovementCouponManagmentDeptApprove")
public class MovementCouponManagmentDeptApprove {
	String TITLE = "MÀN HÌNH DUYỆT LỆNH ĐIỀU ĐỘNG TÀI SẢN SẢN CỦA ĐƠN VỊ";
	...
	public ModelAndView init(HttpServletRequest request)
	{
		...
		mv.setViewName("views/MovementCouponManagmentDeptApprove.jsp");
		return mv;
	}
	...
	public ModelAndView approve(HttpServletRequest request)
	{
		...
		if(Common.isEmpty(status_device))
		{
			...
			MovementCouponUpdateApproveDeptDao updateApprove = new MovementCouponUpdateApproveDeptDao(brCoupn);
			try {
		...
		try {
			...
			mv.addObject("lstBr", lstSearch);
			String ststus_s = "1";
			if("".equals(lst.get(0).getMovement_sticker().trim())) {
				ststus_s = "2";
			}
			if("CM".equals(lst.get(0).getMovement_sticker().trim())) {
				ststus_s = "2";
			}
			if("M".equals(lst.get(0).getMovement_sticker().trim())) {
				ststus_s = "3";
			}
			if("T".equals(lst.get(0).getMovement_sticker().trim())) {
				ststus_s = "1";
			}
			if("TH".equals(lst.get(0).getMovement_sticker().trim())) {
				ststus_s = "4";
			}
			if("BG".equals(lst.get(0).getMovement_sticker().trim())) {
				ststus_s = "1";
			}
			
			for( int i = 0;i < lstSearch.size(); i++) {
				
				if(lstSearch.get(i).getOuter() != null && "on".equals(lstSearch.get(i).getOuter().trim()))
				{
					lstSearch.get(i).getAsset().setStatus(ststus_s);
					lstSearch.get(i).getAsset().setDepartment_cd(lst.get(0).getDept_client().getDept_cd());
					lstSearch.get(i).getAsset().setDelete_Fg("0");
					lstSearch.get(i).getAsset().setCompany_CD(lst.get(0).getCmpn_client().getCompany_cd());
					AssetGeneralInsertDao ins = new AssetGeneralInsertDao();
					ins.excuteData(lstSearch.get(i).getAsset());
				}else {
					lstSearch.get(i).getAsset().setStatus(ststus_s);
					lstSearch.get(i).getAsset().setDepartment_cd(lst.get(0).getDept_client().getDept_cd());
					lstSearch.get(i).getAsset().setCompany_CD(lst.get(0).getCmpn_client().getCompany_cd());
					...
					AssetGeneralMovementDao updex = new AssetGeneralMovementDao();
					updex.excuteData(lstSearch.get(i).getAsset());
				}
	...
	public ModelAndView disApprove(HttpServletRequest request)
	{
		...
		if(Common.isEmpty(reason_not_allow))
		{
			...
			SystemControl sys = new SystemControl(request);
			brCoupn.setUpdate_user(sys.EmployeeCD);
			MovementCouponUpdateDisApproveDeptDao updateApprove = new MovementCouponUpdateDisApproveDeptDao(brCoupn);
			try {
				updateApprove.excute();
